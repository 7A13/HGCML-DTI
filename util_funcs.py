...
def cos_sim(a, b, eps=1e-8):
    ...
    """
    a_n, b_n = a.norm(dim=1)[:, None], b.norm(dim=1)[:, None]#a_n:p-a[4328, 1][4328, 1][2957, 1]yuyi[4328, 1]p-c[4328, 1]; [4328, 1];[20, 1]  b_n:p-a[2957, 1][4328, 1][2957, 1]yuyi[2957, 1]p-c[20, 1];[4328, 1];[20, 1]
    a_norm = a / torch.max(a_n, eps * torch.ones_like(a_n))#p-a:[4328, 16][4328, 334][2957, 334]yuyi[4328, 64]p-c:[4328, 16];[4328, 334];[20, 334]
    b_norm = b / torch.max(b_n, eps * torch.ones_like(b_n))#p-a:[2957, 16][4328, 334][2957, 334]yuyi[2957, 64]p-c:[20, 16];[4328, 334];[20, 334]
    sim_mt = torch.mm(a_norm, b_norm.transpose(0, 1))#矩阵相乘p-a:[4328, 2957][4328, 4328][2957, 2957]yuyi[4328, 2957]p-c:[4328, 20];[4328, 4328];[20, 20]
    return sim_mt
